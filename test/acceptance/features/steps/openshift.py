...
'''
    ...
    def get_resource_info_by_jq(self, resource_type, name, jq_expression, wait=False, interval=5, timeout=120):
        output, exit_code = self.cmd.run(f'{ctx.cli} get {resource_type} {name} -o json | jq -rc \'{jq_expression}\'')
        return output
